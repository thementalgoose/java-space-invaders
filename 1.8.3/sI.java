...
public class sI extends JFrame {
	...
	int timer2 = enemyMovementTimer - 15;
	int timer3 = timer2 - 20;
	int timer4 = timer3 - 25;
	int timer5 = timer4 - 20;
	int timer6 = timer5 - 10;
	int inGame = 2;
	int rows = 10;
	int numberPerRow = 10;
	...
	int frequencyOfBullets = 20;
	int showPercentages = 0;
	int pause = 0;
	
	updatePanel t;
	updatePanelTwo t2;
	randomBulletThread t3;
	...
	public sI() {
		try {
			craft = Toolkit.getDefaultToolkit().getImage(getClass().getResource("/images/craft.png"));
		...
		addKeyListener(new KeyAdapter() {
			public void keyPressed(KeyEvent e) {
				if (e.getKeyCode() == KeyEvent.VK_RIGHT && inGame == 1) {
					if (pause == 0) {
				...
				if (e.getKeyCode() == KeyEvent.VK_LEFT && inGame == 1) {
					if (pause == 0) {
						if (posX <= 10) {} else {
							posX -= increment;
							repaint();
				...
				if (e.getKeyCode() == KeyEvent.VK_SPACE && inGame == 1) {
	...
	public void startGame() {
		t = new updatePanel();
		t.start();
		t2 = new updatePanelTwo();
		t2.start();
		t3 = new randomBulletThread();
		t3.start();
	}
	...
	public int convertConToPer(int i) {
		int a = 0;
		if (i == 1) {
			a = 100;
		}
	...
	public void paint(Graphics g) {
		...
		if (pause == 1) {
			g.drawString("Game paused. Press 'P' to resume", frameX - 200, 40);
		}
		if (inGame == 2) {
			g.drawString("Press 'S' to start the game!", frameX / 2 - 70, frameY / 2 - 10);
		}
		if (inGame == 1) {
			...
			if (showPercentages == 1) {
				for (int i = 0; i < barry.size(); i++) {
					Barriers b = (Barriers) barry.get(i);
					int value = convertConToPer(b.getCondition());
					if (value == 20) {
					...
					g.drawString(value + "%", b.getX() + 6, b.getY() + 32);
					g.setColor(Color.GREEN);
				}
				g.drawString("Active missiles: " + missiles.size(), 70, frameY - 10);
				g.drawString("Active enemy missiles: " + bullet.size(), 180, frameY - 10);
			}
		} 
		if (inGame == 0) {
			for (int i = 0; i < enemies.size(); i++) {
			...
			Image gameover = Toolkit.getDefaultToolkit().getImage(getClass().getResource("/images/gameover.png"));
			ImageIcon gameoverii = new ImageIcon(gameover);
			t.stop();
			t2.stop();
			t3.stop();
			g.setColor(Color.DARK_GRAY);
			g.fillRoundRect((frameX / 2) - (gameoverii.getIconWidth() / 2) - 5, 95, gameoverii.getIconWidth() + 10, gameoverii.getIconHeight() + 53, 10, 10);
			g.setColor(Color.GREEN);
			...
			g.drawString(message, frameX / 2 - 25, frameY / 2 - 30);
		}
	}
	public class updatePanel extends Thread implements Runnable {
		public void run() {
			for (int i = 1; i > 0; i++) {
				...
				if (enemies.size() <= 50) {
					enemyMovementTimer = timer2;
					if (enemies.size() <= 25) {
						enemyMovementTimer = timer3;
						if (enemies.size() <= 12) {
							enemyMovementTimer = timer4;
							if (enemies.size() <= 6) {
								enemyMovementTimer = timer5;
								if (enemies.size() <= 3) {
									enemyMovementTimer = timer6;
									if (enemies.size() <= 1) {
										enemyMovementTimer = 5;
									}
								}
							}
						}
